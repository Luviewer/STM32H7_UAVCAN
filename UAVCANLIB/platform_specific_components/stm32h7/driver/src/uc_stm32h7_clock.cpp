 ...
 * Timer instance
 */
#define TIMX             UAVCAN_STM32H7_GLUE2(TIM, UAVCAN_STM32H7_TIMER_NUMBER)
#define TIMX_IRQn        UAVCAN_STM32H7_GLUE3(TIM, UAVCAN_STM32H7_TIMER_NUMBER, _IRQn)
#define TIMX_INPUT_CLOCK STM32_TIMCLK1
#define TIMX_IRQHandler  UAVCAN_STM32H7_GLUE3(TIM, UAVCAN_STM32H7_TIMER_NUMBER, _IRQHandler)

#if UAVCAN_STM32H7_TIMER_NUMBER >= 2 && UAVCAN_STM32H7_TIMER_NUMBER <= 7
    #define TIMX_RCC_ENR       RCC->APB1LENR
    #define TIMX_RCC_RSTR      RCC->APB1LRSTR
...
namespace clock {
    ...
    void init()
    {
        ...
        // Enable IRQ
        nvicEnableVector(TIMX_IRQn, UAVCAN_STM32H7_IRQ_PRIORITY_MASK);
    ...
    void adjustUtc(uavcan::UtcDuration adjustment)
    {
        ...
        if (adjustment.getAbs() > utc_sync_params.min_jump || !utc_set) {
            ...
            if (!utc_locked) {
                utc_locked = (std::abs(utc_rel_rate_ppm) < utc_sync_params.lock_thres_rate_ppm) && (std::abs(utc_prev_adj) < float(utc_sync_params.lock_thres_offset.toUSec()));
            }
        }
    }
